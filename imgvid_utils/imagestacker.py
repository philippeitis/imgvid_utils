...
class Rename(Transform):
    def __init__(self, dir: str, prefix: str, ext: str, pad: int = 5):
        self.dir = dir
        self.base_name = prefix
        self.ext = ext
        self.pad = pad

    def apply(self, image_data: ImageDataStore) -> ImageDataStore:
        image_data.file_name = str(
            (Path(self.dir) / (self.base_name + image_data.file_name.zfill(self.pad)))
        )
        image_data.ext = self.ext or image_data.ext
        return image_data
...
class GenericImageIterator:
    ...
    def rename(self, directory: str = "./", prefix: str = "", ext="jpg"):
        self._replace_transform(
            Rename(
                directory, prefix, fo.prepend_dot(ext), len(str(self._max_iters - 1))
            )
        )
        return self
    ...
    def write_images(self):
        ...
        :return:
        """
        if find(self.transforms, lambda x: isinstance(x, Rename)) is None:
            raise ValueError(
                f"{self.__class__.__name__} must be initialized with .rename() before calling .write_images()"
            )

        for image_data in self:
            path = Path(image_data.file_name).with_suffix(image_data.ext)
...
class FileIterator(GenericImageIterator):
    def __init__(self, paths, stacking: Stacking = Stacking.default()):
        """
        Samples filtered images from the provided directories in order of appearance.
        Pass in paths to directories containing images, and extension
        of desired image inputs. Will return as many images as needed to fill the stacking.
