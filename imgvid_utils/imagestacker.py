...
class GenericImageIterator:
    ...
    def write_images(self, dir: str = "./", prefix: str = "", ext: Optional[str] = None, pad: Optional[int] = None):
        """
        ...
        for image_data in self:
            ...
            path = (Path(dir) / (prefix + file_name))\
                .with_suffix(ext or image_data.ext or ".jpg")
...
class FileIterator(GenericImageIterator):
    ...
    """
    def __init__(self, paths: Union[List[str], List[List[str]]], stacking: Stacking = Stacking.default()):
